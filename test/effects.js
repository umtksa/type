export const effects = {
    typewriter: (ctx, char, x, y, charProgress, fontSize, width, height, charIndex) => {
        ctx.globalAlpha = charProgress > 0 ? 1 : 0;
        ctx.fillText(char, x, y);
    },
    fade: (ctx, char, x, y, charProgress, fontSize, width, height, charIndex) => {
        ctx.globalAlpha = charProgress;
        ctx.fillText(char, x, y);
    },
    slideUp: (ctx, char, x, y, charProgress, fontSize, width, height, charIndex) => {
        const ease = 1 - Math.pow(1 - charProgress, 3);
        ctx.globalAlpha = charProgress;
        ctx.fillText(char, x, y + (1 - ease) * (fontSize * 0.8));
    },
    popIn: (ctx, char, x, y, charProgress, fontSize, width, height, charIndex) => {
        const c1 = 1.70158;
        const c3 = c1 + 1;
    ...
    blur: (ctx, char, x, y, charProgress, fontSize, width, height, charIndex) => {
        const blurAmount = (1 - charProgress) * (fontSize * 0.2);
        ctx.filter = `blur(${blurAmount}px)`;
        ctx.globalAlpha = charProgress;
        ctx.fillText(char, x, y);
    },
    glitch: (ctx, char, x, y, charProgress, fontSize, width, height, charIndex, textColor) => {
        if (charProgress < 1) {
    ...
    elasticDrop: (ctx, char, x, y, charProgress, fontSize, width, height, charIndex) => {
    ...
    rotateIn: (ctx, char, x, y, charProgress, fontSize, width, height, charIndex) => {
        const ease = 1 - Math.pow(1 - charProgress, 3);
        ctx.globalAlpha = charProgress;
        ctx.translate(x, y);
        ctx.rotate((1 - ease) * Math.PI * 2);
        ctx.scale(ease, ease);
        ctx.fillText(char, 0, 0);
    },
    trackingIn: (ctx, char, x, y, charProgress, fontSize, width, height, charIndex) => {
    ...
    swing: (ctx, char, x, y, charProgress, fontSize, width, height, charIndex) => {
        ...
        const angle = Math.cos(charProgress * Math.PI * 2.5) * (1 - charProgress) * Math.PI / 2;
        ctx.scale(1, Math.max(0.1, Math.cos(angle)));
        ctx.fillText(char, 0, fontSize/2);
    },
    focusBlur: (ctx, char, x, y, charProgress, fontSize, width, height, charIndex) => {
        const ease = 1 - Math.pow(1 - charProgress, 3);
    ...
    wave: (ctx, char, x, y, charProgress, fontSize, width, height, charIndex, textColor, progress) => {
        const ease = 1 - Math.pow(1 - charProgress, 3);
        ctx.globalAlpha = charProgress;
        const waveY = Math.sin(charIndex * 0.5 + progress * Math.PI * 10) * (fontSize * 0.5) * (1 - ease);
        ctx.fillText(char, x, y + waveY);
    },
    fly3D: (ctx, char, x, y, charProgress, fontSize, width, height, charIndex) => {
        const ease = 1 - Math.pow(1 - charProgress, 3);
        ctx.globalAlpha = charProgress;
        ctx.translate(x, y);
        const scaleX = Math.cos((1 - ease) * Math.PI);
        ctx.scale(Math.abs(scaleX), Math.abs(scaleX) * ease);
        ctx.fillText(char, 0, 0);
    },
    shatterIn: (ctx, char, x, y, charProgress, fontSize, width, height, charIndex) => {
        const ease = 1 - Math.pow(1 - charProgress, 3);
    ...
    neonFlicker: (ctx, char, x, y, charProgress, fontSize, width, height, charIndex, textColor) => {
        if (charProgress < 1) {
    ...
    stretchSnap: (ctx, char, x, y, charProgress, fontSize, width, height, charIndex) => {
        const ease = 1 - Math.pow(1 - charProgress, 4);
        ctx.globalAlpha = charProgress;
        ctx.translate(x, y);
    ...
    foldDown: (ctx, char, x, y, charProgress, fontSize, width, height, charIndex) => {
        const ease = 1 - Math.pow(1 - charProgress, 3);
        ctx.globalAlpha = charProgress;
        ctx.translate(x, y - fontSize/2);
